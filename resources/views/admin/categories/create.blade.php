...
@section('content')
    <nav class="breadcrumb-header bg-white justify-content-between p-3 rounded-5">
        <div>
            <ol class="breadcrumb bg-transparent ">
                ...
                <li class="breadcrumb-item text-sm  active" aria-current="page">{{__('category.create')}}</li>
            </ol>
            <h6 class="font-weight-bolder">{{__('category.create')}}</h6>
        </div>
        <div>
    ...
    <div class="row">
        <div class="col-md-6 col-12 mx-auto">
            <form action="{{ route('category.store')}}" method="post" class="card mt-4">
                @csrf
                <div class="card-header border-bottom">
                    <h4 class="mb-0">{{__('category.create')}}</h4>
                    <p>{{__('category.create_subtitle')}}</p>
                </div>
                ...
                <div class="card-body">
                    @foreach(config('settings.locales') as $lang)
                        <div class="form-group">
                            ...
                            <input type="text" name="{{$lang.'[name]'}}" class="form-control" id="name{{$lang}}" value="{{old("{$lang}.name")}}" required>
                            @error($lang.'.name')
